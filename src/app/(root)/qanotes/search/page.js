...
export default async function DatePage({ params, searchParams }) {
    const { date, topicStr, subtopicsStr } = await searchParams;

    if (!(date || topicStr || subtopicsStr)) {
        return (
            <div className="mx-auto p-4">
                <h1 className="text-2xl font-bold mb-4">Search Results</h1>
                <p>Please provide all search criteria.</p>
            </div>
        );
    }

    const topic = topicStr?.filter
        ? topicStr.filter.split(",").filter(Boolean)
        : [];
    const subtopics = subtopicsStr?.filter ? subtopicsStr.filter.split(",").filter(Boolean) : [];
