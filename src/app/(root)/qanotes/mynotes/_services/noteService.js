...
// If date given → only topics/subtopics that have notes on that date
...
// ─── Notes (main query) ───────────────────────────────────────────────────────
// topics    → string[] — if empty, no topic filter
// subtopics → string[] — if empty, no subtopic filter (all subtopics selected)

export async function getNotes({ date = null, topics = [], subtopics = [] } = {}) {
  const conditions = [];

  if (date) {
    conditions.push(eq(notesView.noteDate, date));
  }

  if (topics.length) {
    conditions.push(inArray(notesView.topicName, topics));
  }

  // Only filter subtopics if partial selection (not all)
  if (subtopics.length) {
    conditions.push(inArray(notesView.subtopicName, subtopics));
  }
...
// ─── Subtopic metrics ─────────────────────────────────────────────────────────
// topic: string → filter by topicName
// topic: number → filter by topicId
